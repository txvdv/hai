/**
 * Represents a feature toggle in a feature management system.
 * A feature toggle controls whether a feature should be enabled or not,
 * based on a set of matching rules defined by the `rules` property.
 */

export interface FeatureToggle {
  /**
   * A unique identifier for the feature toggle.
   */
  key: string;
  /**
   * The default value of the feature toggle if no rules match in evaluation.
   * Can be of any data type depending on the context of the feature.
   */
  defaultValue: any;
  /**
   * A list of targeting rules used to determine the value of the feature toggle
...
/**
 * Represents a rule for targeting users or conditions for a feature toggle.
...
export interface TargetingRule {
  ...
  /**
   * The value to be returned for the feature toggle if this rule is matched.
   */
  value: any;
}

/**
 * Represents a condition to be checked during feature toggle evaluation.
 */
...
/**
 * Represents the context for evaluating a feature toggle.
...
export class FeatureToggles {
  private flags: FeatureToggle[];

  constructor(flags: FeatureToggle[]) {
    this.flags = flags;
  }
  /**
   * Evaluates the value of a feature toggle based on its key and an evaluation context.
   * If no rules match, the toggle's default value is returned.
   *
   * @param flagKey - The key of the feature toggle to evaluate.
   * @param context - An evaluation context containing attributes for evaluation.
   * @returns The value of the feature toggle depending on the matched rules, or the default value.
   */
  evaluate(flagKey: string, context: EvaluationContext): any {
    const toggle = this.flags.find((f) => f.key === flagKey);
    if (!toggle) {
