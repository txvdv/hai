...
function z(r, e) {
  ...
  if (t || s) {
    ...
    s > 0 && (n += ` ${s} file${s === 1 ? " is" : "s are"} already cached.`), i.groupCollapsed(n), x("View newly precached URLs.", r), x("View previously precached URLs.", e), i.groupEnd();
  }
}
let N;
function ee() {
...
let Ce;
V(
  new ve(Ne("index.html"), { allowlist: Ce })
);
self.skipWaiting();
_e();
...
self.addEventListener("message", async (r) => {
  if (console.log("Service Worker received message:", r), !r.source) {
    console.error("event.source is null. Unable to respond to the message.");
    return;
  }
  const e = r.data;
  e && e.action === "hello" ? r.source.postMessage({
    status: "success",
